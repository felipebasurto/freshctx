...
export class JsonlTransport {
  ...
  static async serve({ input, output, handle, maxLineBytes = MAX_JSONL_LINE_BYTES }) {
    ...
    const emitTooLarge = () => JsonlTransport.write(output, failure(null, TOO_LARGE));
...
export async function serveJsonLines(options) {
  return JsonlTransport.serve(options);
}
...
async function writeResponse(line, output, handle) {
  ...
  try {
    try {
      ...
      await JsonlTransport.write(output, failure(null, { code: "invalid_json", message: "JSONL line is not valid JSON" }));
      return;
    }
    const result = await handle(request);
    await JsonlTransport.write(output, result);
  } catch (error) {
    const id = request && typeof request === "object" ? request.id : null;
    await JsonlTransport.write(output, failure(id, publicError(error)));
  }
}
