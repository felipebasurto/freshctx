...
Source is public. Do not publish npm packages, create a promotional launch,
or distribute a blog post without Felipe's new explicit authorization.
Existing tags and release history are preserved.
...
Keep `private: true` in product and bridge manifests until an npm release is
authorized.

Benchmarks remain in the separate freshctx-bench checkout and research remains
in freshctx-research. Those repositories stay private. Do not change frozen
reports, research counts, or claims to improve appearances. No unmeasured SOTA,
Pass@1, dollar savings, or closed-host compatibility claims.
