"""OpenHands wiring for FreshCtx. Speaks freshctx/1 JSONL. Fail-closed."""
...
def wrap_llm(llm: Any, bridge: Bridge) -> Any:
    """Rewrite the model-bound message list after condensation, then dispatch.

    OpenHands must run LLMSummarizingCondenser first. This wrapper is the last
    rewrite before the provider call. A rejected plan raises; it does not send
