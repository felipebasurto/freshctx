...
export class Bridge {
  ...
  async read(resultId, { path, offset = 1, limit = 200 }) {
    ...
    // Exclude terminal line separators, as Pi's text read does. They are
    // outside a Tree-sitter declaration and would force a region selector.
  ...
  async rewrite(payload) {
    ...
    for (const replacement of plan.replacements) {
      ...
      // Pi serializes an empty tool result as this literal placeholder.
      const text = result?.content === '(no tool output)' && replacement.expected_sha256 === revisionFor('') ? '' : result?.content;
