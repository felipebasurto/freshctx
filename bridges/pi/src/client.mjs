...
// The pinned product dependency owns the executable, not the host's PATH.
export class Client {
  constructor({ root, timeoutMs = 10000, command = process.execPath, args } = {}) {
    const executable = fileURLToPath(new URL('../bin/freshctx.mjs', import.meta.resolve('freshctx/workspace')));
    this.child = spawn(command, args ?? [executable, 'serve', '--stdio', '--root', root], { stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    this.lines.on('line', line => {
      try {
        ...
        if (reply.protocol !== 'freshctx/1' || typeof reply.ok !== 'boolean') throw new Error('Invalid FreshCtx response');
  ...
  request(op, fields = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.child.stdin.write(JSON.stringify({ ...fields, protocol: 'freshctx/1', id, op }) + '\n', error => {
